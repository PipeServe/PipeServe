...
class LLMAnalysis:
    ...
    def __init__(
        ...
        if self.parallelism_config.sp_size > 1:
            assert self.parallelism_config.sp_size == self.parallelism_config.tp_size, (
        ...
        self.total_num_params_mlp = (
            self.get_num_params_per_layer_mlp() * self.model_config.num_layers
        )
    ...
    def update_parallelism_config(self, parallelism_config: ParallelismConfig) -> None:
        self.parallelism_config = parallelism_config
    ...
    def get_gpu_hbm_bandwidth(self) -> float:
        return self.gpu_config.hbm_bandwidth_in_GB_per_sec * self.hbm_memory_efficiency
    ...
    def get_num_params_per_layer_attn(self) -> int:
        ...
        return (
            2 * self.model_config.hidden_dim** 2
            + 2
    ...
    def get_weight_memory_per_layer(
        ...
        if is_sharded and ds_zero == DSZeRO.STAGE_3:
            ...
            mlp_sharded_dp_size = (
                self.parallelism_config.dp_size / self.parallelism_config.ep_size
            )
        else:
            sharded_dp_size = 1
    ...
    def get_memory_optimizer_state_and_gradient_embedding(
        ...
        memory_optimizer_state_embedding = (
            op_bytes_per_params
            * self.get_num_params_embedding()
            / self.parallelism_config.tp_size
        )
    ...
    def get_memory_optimizer_state_and_gradient_per_layer(
        ...
        memory_optimizer_state_mlp_per_layer = (
            op_bytes_per_params
            * self.get_num_params_per_layer_mlp()
        ...
        memory_optimizer_state_others_per_layer = (
            op_bytes_per_params
            * (
                (
                    self.get_num_params_per_layer_attn()
        ...
        memory_gradient_others_per_layer = (
            1
            * (
                self.get_num_params_per_layer_attn()
                + +self.get_num_params_per_layer_router()
        ...
        memory_gradient_per_layer = (
            memory_gradient_mlp_per_layer + memory_gradient_others_per_layer
        )
    ...
    def get_memory_optimizer_state_and_gradient_last_layernorm(
        ...
        memory_optimizer_state_last_layernorm = (
            op_bytes_per_params
            * self.get_num_params_last_layernorm()
            / self.parallelism_config.tp_size
        )
        ...
        memory_gradient_last_layernorm = (
            master_weights_dtype_bytes
            * self.get_num_params_last_layernorm()
            / self.parallelism_config.tp_size
        )
    ...
    def get_activation_memory_per_layer_attn(
        ...
        if activation_recomputation >= ActivationRecomputation.NORM_ATTN_NORM:
            ...
            return seq_len * batch_size * hidden_dim * bytes_per_activation / sp_size
        ...
        if activation_recomputation == ActivationRecomputation.ATTN_COMPUTE:
            ...
            if flash_attn:
                memory_attn_compute = (
                    (
                        2 * seq_len * batch_size * hidden_dim
        ...
        if is_inference:
            return max(
                3 * bytes_per_activation * seq_len * batch_size * hidden_dim / tp_size,
                memory_attn_compute,
            )
    ...
    def get_activation_memory_per_layer_mlp(
        ...
        bytes_per_1linear_input = bytes_per_gelu_input = bytes_per_2linear_input = (
            bytes_per_activation
        )
        ...
        if self.model_config.moe_num_experts > 1:
            ...
            activation_memory_per_layer_mlp = (
                2 * bytes_per_activation * seq_len * batch_size * hidden_dim / sp_size
            )
            ...
            # dense MLP
            activation_memory_per_layer_mlp = (
                bytes_per_activation * seq_len * batch_size * hidden_dim / sp_size
            )
        ...
        if recompute_gelu and gated_linear_units:
            ...
            activation_memory_per_layer_mlp += (
                ...
                * self.model_config.expansion_ratio
                * seq_len
                * batch_size
                * hidden_dim
                * self.model_config.moe_top_k
    ...
    def get_activation_memory_input_embedding(
    ...
    def get_activation_memory_output_embedding(
        self, batch_size: int, seq_len: int
    ) -> float:
        """Get the memory (in bytes) required to store the activations of output embedding (logits)"""
        return (
    ...
    def get_activation_memory_per_layer(
        ...
        activation_memory_per_layer_attn = self.get_activation_memory_per_layer_attn(
            batch_size,
            seq_len,
            is_inference,
        ...
        activation_memory_per_layer_mlp = self.get_activation_memory_per_layer_mlp(
            batch_size,
            seq_len,
            is_inference,
        ...
        if is_inference:
            activation_memory_per_layer = max(
                activation_memory_per_layer_attn,
                activation_memory_per_layer_mlp,
    ...
    def get_memory_kv_cache_per_layer(
        ...
        if kv_cache_dtype_bytes is None:
            kv_cache_dtype_bytes = self.dtype_config.activation_bits / BITS_PER_BYTE
        ...
        memory_kv_cache_per_layer = (
            2 * batch_size * seq_len * head_dim * num_heads_per_gpu
    ...
    def get_num_flops_fwd_per_layer_attn(self, batch_size: int, seq_len: int) -> int:
        ...
        return (
            ...
            * batch_size
            * seq_len
            * self.model_config.hidden_dim**2
    ...
    def get_num_flops_fwd_per_layer_mlp(self, batch_size: int, seq_len: int) -> int:
        """Get the number of floating point operations (flops) for the forward pass of
    ...
    def get_num_flops_total_attn_compute(self, batch_size: int, seq_len: int) -> int:
    ...
    def get_latency_fwd_per_layer_mlp_moe_alltoall(
        self, batch_size: int, seq_len: int
    ) -> float:
        data_nums = (
            self.model_config.moe_top_k
        ...
        latency = data_bytes / (
            (
                ...
                if self.parallelism_config.ep_size <= 8
                else self.get_inter_node_bandwidth()
            )
            * 10**9
        )
        logger.debug(
    ...
    def get_latency_fwd_per_layer_mlp(
        ...
        alltoall_latency = (
            2 * self.get_latency_fwd_per_layer_mlp_moe_alltoall(batch_size, seq_len)
            if self.model_config.moe_num_experts > 1
            else 0
        )
        ...
        logger.debug(f"alltoall_latency = {round(alltoall_latency * 1000, 3)} ms")
    ...
    def get_latency_fwd_per_tp_comm(
    ...
    def get_latency_fwd_per_layer_shared_dp_comm(self) -> float:
        ...
        bandwidth = (
            self.get_intra_node_bandwidth()
            if (dp_size * tp_size) <= 8
            else self.get_inter_node_bandwidth()
        )

        latency_allgather_params_mlp = time_allgather(
            params_bytes_mlp, dp_size / ep_size, bandwidth * 10**9
        )
    ...
    def get_latency_fwd_per_layer(
        ...
        latency_fwd_per_layer_shared_dp_comm = (
            self.get_latency_fwd_per_layer_shared_dp_comm()
        )

        latency_per_layer = (
            latency_fwd_per_layer_attn
            + latency_fwd_per_layer_mlp
            + 2 * latency_fwd_per_layernorm
            + 2 * latency_fwd_per_tp_comm
        )

        if (
            ds_zero > DSZeRO.STAGE_1
            and latency_fwd_per_layer_shared_dp_comm > latency_per_layer
        ):
            logger.warning(
    ...
    def get_latency_fwd(
        ...
        latency_fwd_breakdown = {
            breakdown_prefix + "latency_fwd_attn": breakdown_per_layer["attn"]
            * num_layers_per_gpu,
            breakdown_prefix + "latency_fwd_mlp": breakdown_per_layer["mlp"]
            * num_layers_per_gpu,
            breakdown_prefix + "latency_fwd_layernorm": breakdown_per_layer["layernorm"]
            * num_layers_per_gpu,
            breakdown_prefix + "latency_fwd_tp_comm": breakdown_per_layer["tp_comm"]
            * num_layers_per_gpu,
            breakdown_prefix + "latency_fwd_sharded_dp_comm": breakdown_per_layer[
                "sharded_dp_comm"
            ]
            * num_layers_per_gpu,
            breakdown_prefix
    ...
    def get_readable_summary_dict(self, summary_dict: dict, title="Summary") -> str:
        ...
        for key, value in summary_dict.items():
            if "latency" in key:
                ...
                "num_tokens" in key
                or "num_params" in key
                or "num_active_params" in key
                or "flops" in key
            ):
    ...
    def output_summary_dict(
        ...
        logger.info(f"Summary written to {os.path.join(output_dir, file_name)}")
    ...
    def inference(
        ...
        if self.model_config.num_layers % self.parallelism_config.pp_size:
            logger.info("num_layers not be divisible by pp_size, taking the floor")
        ...
        (
            ...
            for x in self.get_weight_memory_per_layer(ds_zero, return_breakdown=True)
        ]
        ...
        memory_left = (
            self.gpu_config.mem_per_GPU_in_GB * 1024**3 - weight_memory_per_gpu
        )
        ...
        prefill_activation_memory_output_embedding_batch_size_1 = (
            self.get_activation_memory_output_embedding(1, seq_len)
        )
        ...
        prefill_activation_memory_output_embedding = (
            self.get_activation_memory_output_embedding(batch_size_per_gpu, seq_len)
        )
        ...
        if use_kv_cache:
            ...
            kv_cache_latency = (
                2 * kv_cache_memory_per_gpu / (self.get_gpu_hbm_bandwidth() * 10**9)
            )
            ...
            decode_activation_memory_output_embedding = (
                self.get_activation_memory_output_embedding(batch_size_per_gpu, 1)
            )
            ...
            decode_activation_memory_per_gpu = decode_activation_memory_per_layer
        ...
        if cost_per_gpu_hour:
            ...
            summary_dict.update(
                {
                    "prefill_cost_per_1k_tokens": prefill_cost_per_1k_tokens,
    ...
    def config_batch_size_and_gradient_accumulation_steps(
        ...
        """
        assert_msg = (
            "note that global_batch_size == batch_size_per_gpu *"
            " gradient_accumulation_steps * dp_size"
        )
        dp_size = self.parallelism_config.dp_size * self.parallelism_config.rdp_size
        if global_batch_size and batch_size_per_gpu and gradient_accumulation_steps:
            assert (
                global_batch_size
                == batch_size_per_gpu * gradient_accumulation_steps * dp_size
            ), assert_msg
            ...
            assert (
                global_batch_size % (gradient_accumulation_steps * dp_size) == 0
    ...
    def training(
        ...
        """
        if seq_len is None:
            assert self.model_config.max_seq_len is not None, (
        ...
        if ds_zero == DSZeRO.NONE:
            logger.warning(
                "DeepSpeed ZeRO is disabled, consider using ZeRO to reduce memory usage"
            )
        ...
        (
            ...
            x * num_layers_per_gpu
            for x in self.get_weight_memory_per_layer(
        ...
        ) = self.get_weight_memory_per_layer(
            is_sharded=False, ds_zero=ds_zero, return_breakdown=True
        )

        optimizer_state_memory_per_layer, gradient_memory_per_layer = (
            self.get_memory_optimizer_state_and_gradient_per_layer(
                master_weights_dtype_bytes, other_op_bytes, ds_zero
            )
        )

        optimizer_state_memory_embedding, gradient_memory_embedding = (
            self.get_memory_optimizer_state_and_gradient_embedding(
                master_weights_dtype_bytes, other_op_bytes, ds_zero
            )
        )

        optimizer_state_memory_last_layernorm, gradient_memory_last_layernorm = (
            self.get_memory_optimizer_state_and_gradient_last_layernorm(
                master_weights_dtype_bytes, other_op_bytes, ds_zero
            )
        )

        optimizer_state_memory_per_gpu = (
            optimizer_state_memory_per_layer * num_layers_per_gpu
            + optimizer_state_memory_embedding
            + optimizer_state_memory_last_layernorm
        )
        gradient_memory_per_gpu = (
            gradient_memory_per_layer * num_layers_per_gpu
            + gradient_memory_embedding
            + gradient_memory_last_layernorm
        )
        ...
        estimated_fwd_prefetch_memory_per_gpu = (
            unsharded_weight_memory_embedding + unsharded_weight_memory_per_layer
        )

        estimated_bwd_prefetch_memory_per_gpu = (
            int(fwd_prefetch) + int(bwd_prefetch)
        ) * (unsharded_weight_memory_per_layer)

        estimated_prefetch_memory_per_gpu = max(
            estimated_fwd_prefetch_memory_per_gpu, estimated_bwd_prefetch_memory_per_gpu
        )

        memory_left = (
            self.gpu_config.mem_per_GPU_in_GB * 1024**3
            - weight_memory_per_gpu
            - optimizer_state_memory_per_gpu
        )
        ...
        if (
            memory_left
            - max(estimated_prefetch_memory_per_gpu, gradient_memory_per_gpu)
            < 0
        ):
            logger.warning(
        ...
        (
            ...
            attn_activation_memory_batch_size_1,
        ...
        activation_memory_input_embedding_batch_size_1 = (
            self.get_activation_memory_input_embedding(1, seq_len)
        )
        activation_memory_batch_size_1 += activation_memory_input_embedding_batch_size_1
        activation_memory_output_embedding_batch_size_1 = (
            self.get_activation_memory_output_embedding(1, seq_len)
        )
        activation_memory_batch_size_1 += (
            activation_memory_output_embedding_batch_size_1
        ...
        if (
            memory_left
            - max(estimated_prefetch_memory_per_gpu, loss_bwd_memory_batch_size_1)
            < activation_memory_batch_size_1
        ):
            logger.warning(
        ...
        if batch_size_per_gpu == 1:
            ...
            activation_memory_input_embedding_per_gpu = (
                self.get_activation_memory_input_embedding(batch_size_per_gpu, seq_len)
            )
            activation_memory_output_embedding_per_gpu = (
                self.get_activation_memory_output_embedding(batch_size_per_gpu, seq_len)
            )
        ...
        memory_left = (
            memory_left
            - activation_memory_per_gpu
            - max(estimated_prefetch_memory_per_gpu, loss_bwd_memory)
        )

        num_flops_fwd_total = self.get_num_flops_fwd_total(batch_size_per_gpu, seq_len)
        num_flops_bwd_total = self.get_num_flops_bwd_total(batch_size_per_gpu, seq_len)

        if activation_recomputation == ActivationRecomputation.FULL:
            num_flops_recompute = num_flops_fwd_total
        elif (
            activation_recomputation == ActivationRecomputation.NORM_ATTN_NORM
            or activation_recomputation == ActivationRecomputation.ATTN
        ):
            num_flops_recompute = (
                self.get_num_flops_fwd_per_layer_attn(batch_size_per_gpu, seq_len)
                * self.model_config.num_layers
            )
        ...
        latency_per_iter_using_flops = (
            latency_per_micro_batch_using_flops * gradient_accumulation_steps
        )
        ...
        if activation_recomputation == ActivationRecomputation.FULL:
            ...
            latency_recompute = (
                num_layers_per_gpu
                * self.get_num_flops_total_attn_compute(batch_size_per_gpu, seq_len)
                / (
                    (self.parallelism_config.tp_size * self.parallelism_config.pp_size)
                    * self.get_TFLOPS_per_gpu()
                    * 1e12
        ...
        if total_num_tokens is not None:
            ...
            total_training_latency_using_flops = (
                latency_per_iter_using_flops * num_iters
            )
            logger.info(
        ...
        summary_dict = {
            ...
            "mlp_activation_quant_bits": mlp_activation_quant_bits,
            "mlp_recompute_gelu": mlp_recompute_gelu,
            "mlp_gated_linear_units": self.model_config.mlp_gated_linear_units,
            ...
            "unsharded_weight_memory_embedding": unsharded_weight_memory_embedding,
            "unsharded_weight_memory_per_layer": unsharded_weight_memory_per_layer,
            "unsharded_weight_memory_attn_per_layer": unsharded_weight_memory_attn_per_layer,
            "unsharded_weight_memory_mlp_per_layer": unsharded_weight_memory_mlp_per_layer,
            "unshared_weight_memory_layernorm": unshared_weight_memory_layernorm,
            "gradient_memory_per_gpu": gradient_memory_per_gpu,
            "optimizer_state_memory_per_gpu": optimizer_state_memory_per_gpu,
            "(weight+op_state)_memory_per_gpu": optimizer_state_memory_per_gpu
            + weight_memory_per_gpu,
            "estimated_fwd_prefetch_memory_per_gpu": estimated_fwd_prefetch_memory_per_gpu,
            "estimated_bwd_prefetch_memory_per_gpu": estimated_bwd_prefetch_memory_per_gpu,
            "loss_bwd_memory": loss_bwd_memory,
            "activation_memory_batch_size_1": activation_memory_batch_size_1,
            "activation_memory_per_gpu": activation_memory_per_gpu,
            "activation_memory_attn_per_gpu": activation_memory_attn_per_gpu,
            "activation_memory_mlp_per_gpu": activation_memory_mlp_per_gpu,
            "activation_memory_layernorm_per_gpu": activation_memory_layernorm_per_gpu,
            "activation_memory_input_embedding_per_gpu": activation_memory_input_embedding_per_gpu,
            "activation_memory_output_embedding_per_gpu": activation_memory_output_embedding_per_gpu,
            "(weight+op_state+act)_memory_per_gpu": optimizer_state_memory_per_gpu
            + weight_memory_per_gpu
            + activation_memory_per_gpu,
            "(weight+op_state+grad)_memory_per_gpu": self.weight_grad_op_state_memory_per_gpu,
            "estimated_peak_memory_per_gpu": optimizer_state_memory_per_gpu
            + weight_memory_per_gpu
...
def train(
    ...
    if total_num_gpus and dp_size:
        assert total_num_gpus % (dp_size * tp_size * pp_size) == 0, (
            f"total_num_gpus {total_num_gpus} must be divisible by dp_size * tp_size * pp_size which is {dp_size * tp_size * pp_size}"
        )
        rdp_size = int(total_num_gpus / (dp_size * tp_size * pp_size))
    elif total_num_gpus:
        assert total_num_gpus % (tp_size * pp_size) == 0, (
            "dp_size is not specified, assuming total_num_gpus = dp_size * tp_size * pp_size, total_num_gpus must be a multiple of tp_size * pp_size"
        )
        dp_size = total_num_gpus // (tp_size * pp_size)
    elif dp_size:
        total_num_gpus = dp_size * tp_size * pp_size
        logger.info(
            "total_num_gpus is not specified, assuming total_num_gpus = dp_size * tp_size * pp_size"
        )
    else:
        dp_size = 1
        logger.info(
            "neither dp_size or total_num_gpus is specified, assuming dp_size = 1"
        )
    logger.info(
